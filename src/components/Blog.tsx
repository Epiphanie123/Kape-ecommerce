...
import image1 from '../assets/bloga.jpg';
import image2 from '../assets/blogb.jpg';
import image3 from '../assets/blogc.jpg';
import image4 from '../assets/blogd.jpg';
import image5 from '../assets/bloga.jpg';

const blogImages = [image1, image2, image3, image4, image5];
...
const commentsData = blogs.map((b) => ({
  blogId: b.id,
                ...
                >
                  ...
                  <div className="relative h-56 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-red-400 text-gray-800 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide">
                        {blog.category}
                      </span>
                    </div>
                    <div className={`absolute bottom-4 left-4 right-4 text-white transition-all duration-300 transform ${
                      ...
                      <button className="bg-red-400 text-gray-800 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-red-400 transition-colors">
                        Read More
                      </button>
                    </div>
                    <div className="absolute top-4 right-4">
                      <span className="bg-black/70 text-white px-2 py-1 rounded text-xs">{blog.readTime} min read</span>
                    </div>
                  </div>
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="text-xl font-bold text-gray-800 mb-3 line-clamp-2 group-hover:text-red-400 transition-colors">
                      {blog.title}
                    </h2>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                      ...
                      <div className={`w-2 h-2 rounded-full transition-all duration-300 ${
                        hoveredCard === blog.id ? 'bg-red-400 scale-150' : 'bg-gray-300'
                      }`}></div>
                    </div>
                  </div>
                </article>
                ...
                return (
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg font-semibold transition-all duration-200 ${
                      currentPage === pageNum
                        ? "bg-red-400 text-gray-800 shadow-lg scale-110"
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-1">
                <button
                  ...
                  className={`py-3 px-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    sidebarTab === "recent" ? "bg-red-400 text-gray-800 shadow-md" : "text-gray-600 hover:bg-gray-50"
                  }`}
                >Recent</button>
                <button
                  onClick={() => setSidebarTab("popular")}
                  className={`py-3 px-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    sidebarTab === "popular" ? "bg-red-400 text-gray-800 shadow-md" : "text-gray-600 hover:bg-gray-50"
                  }`}
                >Popular</button>
                <button
                  onClick={() => setSidebarTab("comments")}
                  className={`py-3 px-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    sidebarTab === "comments" ? "bg-red-400 text-gray-800 shadow-md" : "text-gray-600 hover:bg-gray-50"
                  }`}
                >Comments</button>
              </div>
            </div>
            ...
            <div className="bg-white shadow-lg rounded-2xl p-6">
              ...
              <div className="space-y-4">
                {sidebarTab === "recent" && recentBlogs.map(b => (
                  <div key={b.id} className="group cursor-pointer">
                    <div className="flex gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-800 group-hover:text-red-400 transition-colors line-clamp-2">{b.title}</p>
                ...
                {sidebarTab === "popular" && popularBlogs.map(b => (
                  <div key={b.id} className="group cursor-pointer">
                    <div className="flex gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      <Flame size={14} className="text-red-400 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-gray-800 group-hover:text-red-400 transition-colors line-clamp-2">{b.title}</p>
                ...
                {sidebarTab === "comments" && commentsData.map((c, i) => (
